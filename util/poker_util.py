...
def card_count(board): #count of each numbers in the board
...
def suite_count(board): #count of each suit in the board
...
def calculate_max_streak(board): # is continuous function return max streak
  ...
  for i in range(len(nums)-1):
    if nums[i+1]==nums[i]+1:
      ...
      streak = 1
  if nums[0]==1 and nums[-1]==13:  # this condition deals with the special case where Ace forms a streak with kings rather than 2.
...
def calculate_hand(board):
  ...
  """
  n_counts = card_count(board)
  ss_counts = suite_count(board)
  num_straight = calculate_max_streak(board)
...
def calculate_possibility(board, my_hand, hands_num, cards):
